...
def rotate_by_matrix(x, y, theta_degrees, center):
...
class MembraneSubtract:
    def __init__(self, control_points, image, psi, origin_x, origin_y, pixel_size):
    ...
    def mem_subtract(self):
        control_points = self.control_points_trasf(self.control_points, psi, origin_x, origin_y)
        fitted_curve_points, t_values = generate_curve_within_boundaries(control_points, self.image.shape, 0.005)
        ...
        raw_image_average_2d = self.average_2d(self.image, fitted_curve_points, points_along_normal(control_points, t_values).get(), raw_image_average_1d_lst, self.mem_dist)
        ...
        mem_subtracted = self.image_gpu - raw_image_average_2d * (scaling_factor_lst[np.argmin(l1_np_array)] + bias)
        return mem_subtracted.get()
